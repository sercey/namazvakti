...
{
    $.get('json.php', { islem: 'ulke' }, function(data){
...
function sehirleri_al()
{
    ...
    $.get('json.php', { islem: 'sehir', ulke: ulke_adi }, function(data){
...
function ilceleri_al() {
    ...
    if ( ulke_adi == 2 || ulke_adi == 33  || ulke_adi == 52 ) {
        $.get('json.php', { islem:'ilce', sehir: sehir_adi }, function(data){
            var response = $.parseJSON(data);
            if (response.durum == 'basarili') {
                $.each(response.veri, function(i,item){
                    var newOption = $('<option />');
...
function vakti_al() {
    ...
    $.get('json.php', { islem:'vakit', ilce: ilce_adi, sehir: sehir_adi, ulke: ulke_adi }, function(data){
